...
export default function App() {
  ...
  const toggle = () => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, ([activeTab]) => {
      if (!activeTab?.id || activeTab.id === undefined) return

      // Проверка на защищенные протоколы (url может быть пустой без прав "tabs")
      const url = activeTab.url || ""
      const isRestricted = url.startsWith("chrome://") || url.startsWith("edge://")
      if (isRestricted || !url) return
      const sendMessage = () => {
        chrome.tabs.sendMessage(activeTab.id as number, { type: "TOGGLE_DARK", enabled: newValue }, () => {
          if (chrome.runtime.lastError) console.warn("seng msg error", chrome.runtime.lastError)
        })
      }
  ...
  return (
    <div className="bg-radial-primary mx-auto flex min-h-screen max-w-90 flex-col items-center justify-start gap-4 p-4">
      ...
      <Button variant="destructive" size="lg" className="h-10 w-full text-foreground" onClick={toggle}>
